...
status_t dynl_link_to_static_elf(elf_file* source, size_t sourceLocation, elf_loaded_image* targetimage){
	...
	if(dynstrsec == 0)
		FERROR(TSX_ELF_MISSING_SECTION);
	elf_sh* relsec = elf_get_sh_entry(source, ".rela.plt");
	elf_sh* reldynsec = elf_get_sh_entry(source, ".rela.dyn");
...
status_t dynl_link_to_static_elf_symlink(elf_file* source, size_t sourceLocation, elf_loaded_image* targetimage, elf_sh* section, elf_symtab* dynsym, char* dynstr){
	status_t status = 0;
	dynl_rela* rela = (dynl_rela*) (section->sh_offset + (size_t) source);
	for(int i = 0; i < section->sh_size / sizeof(dynl_rela); i++){
		if(DYNL_R_x_TYPE(rela[i].r_info) == DYNL_R_x_JUMP_SLOT || DYNL_R_x_TYPE(rela[i].r_info) == DYNL_R_x_GLOB_DAT){
			size_t codeOffset = rela[i].r_offset + sourceLocation;
			char* name = dynstr + dynsym[DYNL_R_x_SYM(rela[i].r_info)].st_name;
		...
		}else if(DYNL_R_x_TYPE(rela[i].r_info) == DYNL_R_x_RELATIVE){
			size_t codeOffset = rela[i].r_offset + sourceLocation;
			*((size_t*)(codeOffset)) = rela[i].r_addend + sourceLocation;
		}else{
...
status_t dynl_link_image_to_image(elf_loaded_image* sourceimage, elf_loaded_image* targetimage){
	...
	if(dynstrsec == 0)
		FERROR(TSX_ELF_MISSING_SECTION);
	elf_loaded_image_section* relsec = elf_get_section_from_image(sourceimage, ELF_LI_SECTION_RELAPLT);
	elf_loaded_image_section* reldynsec = elf_get_section_from_image(sourceimage, ELF_LI_SECTION_RELADYN);
	if(dynsymsec->entsize != sizeof(elf_symtab))
		FERROR(TSX_ERROR);
	elf_symtab* dynsym = dynsymsec->location;
	char* dynstr = dynstrsec->location;
	if(relsec){
		if(relsec->entsize != sizeof(dynl_rela))
			FERROR(TSX_ERROR);
		status = dynl_link_image_to_image_symlink(sourceimage, targetimage, relsec, dynsym, dynstr);
		CERROR();
	}
	if(reldynsec){
		if(reldynsec->entsize != sizeof(dynl_rela))
			FERROR(TSX_ERROR);
...
status_t dynl_link_image_to_image_symlink(elf_loaded_image* sourceimage, elf_loaded_image* targetimage, elf_loaded_image_section* section, elf_symtab* dynsym, char* dynstr){
	status_t status = 0;
	dynl_rela* rela = section->location;
	for(int i = 0; i < section->length / sizeof(dynl_rela); i++){
		if(DYNL_R_x_TYPE(rela[i].r_info) == DYNL_R_x_JUMP_SLOT || DYNL_R_x_TYPE(rela[i].r_info) == DYNL_R_x_GLOB_DAT){
			size_t codeOffset = rela[i].r_offset + sourceimage->loadLocation;
			char* name = dynstr + dynsym[DYNL_R_x_SYM(rela[i].r_info)].st_name;
			bool symAtTarget = true;
			elf_symtab* targetSymbol = elf_get_symtab_entry_from_image(targetimage, name);
			if(targetSymbol == 0 || targetSymbol->st_value == 0){
			...
			*((size_t*)(codeOffset)) = targetOffset;
			...
			size_t codeOffset = rela[i].r_offset + sourceimage->loadLocation;
			*((size_t*)(codeOffset)) = rela[i].r_addend + sourceimage->loadLocation;
		}else{
			FERROR(TSX_UNIMPLEMENTED);
		}
	}
